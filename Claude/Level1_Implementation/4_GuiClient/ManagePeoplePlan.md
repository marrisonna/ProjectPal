...
**Status: built and verified end-to-end against the real REST API (`D1.4-83`–`D1.4-89`, `D-DM-12`/`D-DM-13`).** Written before any code, per the user's own request, following the same before-code-review pattern `ProjectDetailPlan.md`/`TaskGridPlan.md`/`ComponentDetailPlan.md`/`SearchPlan.md` (this folder) already established. This document grew from a single-screen "Manage People" plan into two related screens once the user confirmed Team Management (originally deferred to Level 2 by `D-DM-11`) is needed for Level 1 too, and asked for it to be folded into this same piece of work (§5). All open questions (§9) are answered — see that section for the final list, and `Plan.md` §10/`Requirements/DomainModel.md`'s own `D-DM-12` for the full decision text. §4.2/§5.2/§5.3 below describe Colour's *current* location (Team Management, not Manage People) — see the closing note at the end of this status block for why it moved after this doc was first written.

Server changes (§6) are all built in `rest-api/app/routes/teams.py`, with `tests/test_person_lifecycle.py`'s own 9 tests (full suite: 63 passed). GUI: `features/people/ManagePeoplePage.tsx` and `features/teams/TeamManagementPage.tsx`, wired into `App.tsx`/`AppShell.tsx`/`windowNav.ts` per §7.3, `tsc`/`oxlint`/`vitest`/`npm run build` all clean. One deviation from §7.1's own sketch: `useUpdatePerson(personId)`/`useUpdatePersonRole(personId, teamId)` were written as unbound `useUpdatePersonField()`/`useUpdatePersonRoleField()` instead (taking the id(s) per call) — both screens' own grids edit a different row on every call, the same shape `useUpdateTaskField()` already established for the same reason, which wasn't fully appreciated until actually building the grid. One incidental fix made along the way: `components/DenseDataGrid.tsx`'s `DenseSingleSelectEditCell` (reused here for Team Management's own Role column) had a pre-existing bug — it called `useGridApiRef()` fresh inside the cell renderer, which is just a disconnected `useRef(null)`, not the live grid API, so every such edit (including TaskGrid's own Status/Priority/Owner/Requestor/Task Type) would have silently failed to commit; fixed to use `props.api` instead.

**Post-build perf investigation** (interactive browser testing, once the user actually clicked through it): editing a cell in either new screen initially froze the whole window solid for 4-5 seconds, with a second, smaller delay before another open window picked up the change. Root-caused and fixed in `components/DenseDataGrid.tsx` (shared with TaskGrid/Search, not just these two screens):
- Both new screens' trailing column (`colour`/`is_resource`) now gets `flex: 1` — without it, MUI DataGrid leaves unused window width as an unlabelled "phantom" filler column, the same bug Search's own Description column already fixed the same way.
- `useDenseGridColumns`'s own per-column auto-width measurement re-measured *every row's* content via a real DOM element on *every* render — each call forces a synchronous layout reflow, and doing that for every row on every keystroke/edit is what actually froze the tab (a genuine network wait never blocks the main thread; only synchronous work does). Fixed with a per-row width cache, keyed by a new required `getRowId` parameter: a row whose content hasn't changed since it was last measured is never remeasured, and the column's own overall width is still recomputed exactly (via `Math.max` over the — mostly cached — per-row numbers) on every render, so it still narrows correctly when a value gets shorter, not just a one-way growth ratchet.
- `useUpdatePersonField`/`useUpdatePersonRoleField` were first written matching `useUpdateTaskField`'s optimistic-update pattern (`beginOptimisticUpdate`/`updateEverywhere`), which turned out to make things worse here: each of `onMutate`/`onSuccess` writing the query cache once meant 2-3 full expensive grid rebuilds per edit instead of one. Reverted to plain `invalidateEverywhere`.
- `getFilteredRows()` used `Array.prototype.filter`, which always allocates a new array even when nothing was filtered out — so an incidental re-render (e.g. the mutation's own pending→success transition) hands the grid a *new* `rows` identity despite the data being unchanged, and MUI DataGrid treats that as "the data changed," re-syncing its internal row model and clobbering the value `processRowUpdate` had just committed. This was the actual cause of a very specific, since-fixed symptom: the cell exiting edit mode showing the *old* value for half a second before flipping to the new one. Fixed by returning `rows` itself, unchanged, whenever filtering removed nothing.

The remaining delay after all four fixes (~1-2s combined, spread across the edit committing and the other window picking it up) turned out to be almost entirely a `vite dev`-only artifact — React's `StrictMode` double-invokes every render, and the dev build carries extra runtime checks a real deployment doesn't. Retested against a real production build (`npm run build && npm run preview`): **~0.25s end-to-end**, confirmed by the user as no longer feeling sluggish. GUI verification for the rest of this feature (§8's own checklist — nav gating, the self-demotion guard, deletion reference-checking, the Team switcher, the never-leaderless guard) is still manual-review/build/lint/type-check only; only this specific perf regression was interactively confirmed fixed.

**Colour moved from Person to PersonRole** (`D-DM-13`/`D1.4-88`, decided and built shortly after the above): the user asked for the Gantt bar colour to be per-Team rather than one org-wide value per Person, mirroring nickname/role/is_resource's own earlier move. `001_initial_schema.sql` edited directly and the dev DB volume rebuilt from scratch (both seed files carry each Person's existing colour over to their own `person_role` row; `002_team2_from_v1.sql`'s CSS colour names normalised to hex). `teams.py`: `colour` removed from Person's request models/column list, added to `WritePersonRoleRequest`/`UpdatePersonRoleRequest` alongside a new `_PERSON_ROLE_COLUMNS` constant (replacing four hand-inlined column lists). GUI: the Colour column, `ColourSwatch`, and `ColourEditCell` moved from `ManagePeoplePage.tsx` to `TeamManagementPage.tsx`; the "Add Person to Team" dialog now pre-fills a computed default colour (`lib/colourPalette.ts`'s `suggestNextColour` — a curated palette, falling back to golden-angle hue generation once exhausted), always still editable. 3 new/updated server tests, full suite (66) green; `tsc`/`oxlint`/`vitest`/`build` all clean on the GUI side. No separate design doc for this one — confirmed with the user inline given the contained scope.

**Manage People's own action-icon column fixed and Team creation/renaming added** (`D1.4-89`, decided and built shortly after the above): the icon column was clipping both icons (too narrow for two, MUI's own IconButton padding); widened, tightened padding, and the Delete icon originally only showed for a Person the eight-table reference check (D-DM-12) actually allowed deleting, rather than always showing it and explaining a 409 afterward (matching TaskGrid's own `canDeleteRow` convention at the time) — both icons darkened to `rgba(0,0,0,0.87)` specifically here (they're the whole cell's content, not a secondary affordance beside other content, unlike the muted `rgba(0,0,0,0.28)` used everywhere else at the time), and the column now has a real header ("Actions"). **Superseded by `D1.4-100`:** the Delete icon is always shown now, greyed out and genuinely disabled (not hidden) when not deletable — see that entry, and §4.5 below (which already described "always present" — this brought the actual code back in line with it). Separately: Team creation (`POST /team`) and renaming (`PATCH /team/{id}`) had no GUI entry point anywhere — added to `TeamManagementPage.tsx`'s own "no id" picker view (admin-only, mirroring Manage People's own "New Person" button), via `CreateOrRenameTeamDialog` (mirrors `CreateOrRenameProjectDialog.tsx`'s create/rename duality) and two new hooks, `useCreateTeam`/`useRenameTeam`. `tsc`/`oxlint`/`vitest`/`build` all clean; no separate design doc, same reasoning as `D1.4-88`.
...
(**Colour** was originally planned as a column here too, but moved to Team Management instead almost immediately after this screen shipped — `D-DM-13`/`D1.4-88` — once the user asked for it to be per-Team rather than per-Person-per-Organisation. See §5.2.)
...
- **Colour** — governed, a small colour swatch plus a native `<input type="color">` for editing (moved here from Manage People, `D-DM-13`/`D1.4-88`, once the user asked for it to be per-Team rather than per-Person-per-Organisation).
...
A small `Dialog` — a picker of active People **not already on this Team** (`usePeople()` filtered against this Team's own current `person_role` rows), plus initial Role, Is Resource, and Colour, calling `POST /person-role`. Deliberately not a "create a new Person" flow — per `D-DM-4`, a Team Lead cannot create Person records at all; only an organisation admin can (Manage People, §4.3), so this dialog only ever picks among People who already exist. Colour (`D-DM-13`/`D1.4-88`) is pre-filled with a computed default — the next colour from a small curated palette not already used by anyone else on this Team, falling back to a generated one (golden-angle hue rotation) once the palette is exhausted (`lib/colourPalette.ts`) — always still editable before confirming, never silently forced.
