...
**Manage People's own action-icon column fixed and Team creation/renaming added** (`D1.4-89`, decided and built shortly after the above): the icon column was clipping both icons (too narrow for two, MUI's own IconButton padding); widened, tightened padding, and the Delete icon now only shows for a Person the eight-table reference check (D-DM-12) actually allows deleting, rather than always showing it and explaining a 409 afterward (matching TaskGrid's own `canDeleteRow` convention) — both icons darkened to `rgba(0,0,0,0.87)` specifically here (they're the whole cell's content, not a secondary affordance beside other content, unlike the muted `rgba(0,0,0,0.28)` used everywhere else), and the column now has a real header ("Actions"). Separately: Team creation (`POST /team`) and renaming (`PATCH /team/{id}`) had no GUI entry point anywhere — added to `TeamManagementPage.tsx`'s own "no id" picker view (admin-only, mirroring Manage People's own "New Person" button), via `CreateOrRenameTeamDialog` (mirrors `CreateOrRenameProjectDialog.tsx`'s create/rename duality) and two new hooks, `useCreateTeam`/`useRenameTeam`. `tsc`/`oxlint`/`vitest`/`build` all clean; no separate design doc, same reasoning as `D1.4-88`.
