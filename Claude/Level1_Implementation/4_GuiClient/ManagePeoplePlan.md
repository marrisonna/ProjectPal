...
**Status: built and verified end-to-end against the real REST API (`D1.4-83`–`D1.4-87`, `D-DM-12`).** Written before any code, per the user's own request, following the same before-code-review pattern `ProjectDetailPlan.md`/`TaskGridPlan.md`/`ComponentDetailPlan.md`/`SearchPlan.md` (this folder) already established. This document grew from a single-screen "Manage People" plan into two related screens once the user confirmed Team Management (originally deferred to Level 2 by `D-DM-11`) is needed for Level 1 too, and asked for it to be folded into this same piece of work (§5). All open questions (§9) are answered — see that section for the final list, and `Plan.md` §10/`Requirements/DomainModel.md`'s own `D-DM-12` for the full decision text.
