...
**Status: built and verified** against the real REST API and real seeded data (headless-CDP, `Plan.md` §7.2 item 15) — every decision below reflects what was actually implemented, not just proposed, including the post-ship refinements in `D1.4-47` (Task-visibility default, sibling-Project ordering). `V2/UserDocumentation/ProjectDetailView.md` (the end-user document, `Plan.md` §7.3) exists. Not yet built: Project List itself (§1's own scoping), and everything in §4.9 (deferred to Stage 5, `D1.4-40`).
...
Each row is either a sub-Project or a Task directly under this Project — this Project's own visible Tasks are listed first, then its sub-Projects (`D1.4-47`, matching the Gantt view's own default ordering, `ganttLayout.ts`'s `collectRows`, exactly — not the reverse):

- **A sub-Project row** shows its name (click opens that sub-Project's own Project Detail window, replacing V1.2's drag-source/click duality — click alone is enough since dragging is deferred), an expand/collapse arrow (children loaded from the already-fetched `useProjects()`/`useTasks()` reference data — no separate lazy-load request needed at Level 1's data scale, unlike V1.2's lazy DB fetch), and, if `hasRoleAtLeast(person, teamId, "LeadUser")`, a small trash icon to delete it (§4.8) and a small pencil icon to rename it (§4.6). Sibling sub-Projects (at any depth, including the top-level Projects shown in "Top Level Projects" mode) are ordered by Priority, highest first, then alphabetically within the same Priority (`D1.4-47`) — the same Priority-to-number mapping Urgency already uses (`priorityWeight`, `lib/schedule.ts`), not a separately-invented order.
- **A Task row** shows its own description (click opens that Task's own Task Detail window, `openItemWindow("tasks", id)`) and its computed Urgency, colour-coded exactly as in All Tasks/the Gantt view (`computeTaskRowColour`) — giving an at-a-glance read of this Project's own Task health without needing to open All Tasks separately. Sorted by computed start date, same as the Gantt view's own default.
- A **Task-visibility filter** (None/Open/All radio group, or an equivalent MUI toggle) above the tree, controlling which Tasks the tree shows, at every level and in every mode (including "Top Level Projects" — there is no separate, mode-based rule hiding Tasks; this filter alone decides) — a pure client-side display filter (§2.2), defaulting to **"None"** (`D1.4-47`, revised from an earlier "Open" default — the initial view of this screen, and especially of "Top Level Projects," reads more like a pure Project-structure browser with Tasks hidden by default, rather than a Task-health dashboard).
