...
- **The API base URL must be configurable, not hardcoded.** Today (`docker-compose.yml`) the REST API is reachable directly at `http://localhost:8000`; once Phase 6 exists, the GUI and API will share one origin (Caddy's `/api/*` prefix). Building the API client against a single configurable base URL (an environment variable baked in at build time, e.g. `VITE_API_BASE_URL`) makes that later switch a one-line config change rather than a code change.
