...
Urgency (§2.1) needs to be surfaced here too — colour-coding bars by urgency is a plausible use of it, matching V1.2 — but it's no longer new work by this point: `5_UrgencyCalculation/Plan.md` already built it as a shared utility (`gui-client/src/lib/schedule.ts`'s `computeUrgency`/`computeUrgencyColour`) and it's already wired into the All Tasks grid and Task Detail (§2.1's note added during that phase). This stage just reuses it, the same way it reuses `buildScheduleGraph`/`getTaskSchedule` for the bars' own Start/End dates.
...
**Not yet testable:** the Gantt/plan view (Stage 3) and the remaining screens (Stage 4) aren't built yet.
