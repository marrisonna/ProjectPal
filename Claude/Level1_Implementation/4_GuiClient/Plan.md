...
**What this stage explicitly defers**, each a legitimate separable follow-up rather than a corner cut: the resource-loading swimlane (`ResourceLevel.cs`'s algorithm, a second rendering problem from the bars themselves — worth its own pass once the core view lands); V1.2's People/Status/Priority colour-mode toggle (V2 ships Urgency-colouring only for now, consistent with what's already built elsewhere); the Component-scoped Gantt variant (Stage 4); and the secondary per-Resource workload-over-time chart (a standard bar/area chart, e.g. via `recharts` — genuinely a different, lower-risk problem from the Gantt bars, doesn't need this stage's own evaluation).
