...
- **Q1.4-17: Which compact Task Detail layout direction (if any) should be built?** §3.11 documents the general principles; three concrete mockups exploring them were drafted and published for review (a Claude Design canvas, compared against the actual `V1.2` screenshot): "dense header + tabs" (closest to today's structure, just compacted), "metadata sidebar" (a two-column split separating identity/scheduling fields from content), and "accordion sections" (the most aggressive on default compactness, via independently-collapsible sections). Not yet decided, and not necessarily an as-is pick — a hybrid, or a fourth direction, is equally on the table.
