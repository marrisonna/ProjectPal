...
**Post-implementation fix (round 1):** the first version of "Show Filter" fell back to DataGrid's own default column-header renderer whenever hidden, instead of `FilterableHeader`. That changed the *label's own* appearance depending on filter visibility — its default renderer centres a single line within the header's fixed height using a lighter font weight, where `FilterableHeader`'s own label is top-aligned and bold — reported directly as "the sort row's height increases and its font goes from bold to not bold" when hiding the filter. Fixed by never switching renderers at all: `FilterableHeader` now always renders (`filterRowVisible` prop replaces the on/off `renderHeader` branch), and the filter box + button beneath the label toggled `visibility`/`pointerEvents` while staying in the layout, so the label above never shifted position, size, or weight.

**Post-implementation fix (round 2):** that first fix kept the filter row's own 20px of space reserved (just invisible) even while hidden, which the user then reported as "a large empty space underneath the column sort headers" — directly working against the actual point of hiding the filter, which is to give that space back to data rows. Fixed by no longer reserving that space at all: `FilterableHeader`'s own filter-row `Box` is now genuinely omitted (not merely hidden) when `filterRowVisible` is `false`, and `TaskGrid`'s own `columnHeaderHeight` (a single grid-wide constant, so it has to cover every column uniformly) now itself shrinks — `HEADER_HEIGHT` (48) while filters are shown, a new `HEADER_HEIGHT_NO_FILTER_ROW` (24, sized to fit the label alone with its own existing padding) while hidden. The label's own styling (font weight, padding) is untouched by either fix; only the header row's own total height and whether the filter row's own space is reserved have ever changed. Verified via headless CDP: the header cell's rendered height goes from 48px to 24px when hiding the filter (and back to exactly 48px on re-showing), the label is confirmed unclipped at 24px, and the first data row visibly moves up by the reclaimed 24px, matching the "give the space back to data" ask precisely.
...
**Approved (`D1.4-65`):** confirmed by the user — `AllTaskPage.tsx` and `AllTaskOrigPage.tsx` are equivalent. Not yet executed as of this decision (a documentation-only turn); the actual removal is a small, mechanical follow-up: delete `AllTaskOrigPage.tsx`, its `/tasks-orig` route (`App.tsx`), and the "Tasks (orig)" button (`AppShell.tsx`).
