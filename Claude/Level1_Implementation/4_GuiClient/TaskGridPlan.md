...
**Status: design complete, not yet implemented.** Written before any code, per the user's request, following the same before-code-review pattern `ProjectDetailPlan.md` (this folder) already established. Everything below reflects a design worked through and explicitly agreed turn-by-turn with the user, not a first draft — see `Plan.md` §10's `D1.4-48`–`D1.4-53` for the condensed decision log this document expands on.
