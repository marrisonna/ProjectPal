...
- **Deployment/packaging** — something a customer site can stand up without a dedicated ops team (Docker container, simple installer, or VM image). Reachability for trial users is via a Cloudflare Tunnel (`6_HttpsReverseProxy/Plan.md`'s `D1.6-1`), a deliberate, documented departure from "stays inside the customer's own network" (§1) made specifically to avoid any customer IT involvement — see that decision for the full trade-off.
