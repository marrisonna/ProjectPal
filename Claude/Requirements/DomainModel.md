...
- **D-DM-14**<br>
  **Question:** `2_RestApi/Plan.md` §2.1 built no `DELETE /team/{id}` route at all — Team deletion was left "not yet decided." The user asked for a way to undo creating a Team by mistake, the same "undo a just-made mistake" motivation `D-DM-12` already settled for Person. Should Team deletion come back, and on what terms?<br>
  **Decision:** yes, narrowly, but on genuinely different terms from Person's own check — not a straight mirror. Rejected (409, naming what's attached) if the Team has any Project or Component; **not** rejected for having PersonRole rows (Team membership), which are cascaded away with the Team instead. This asymmetry with `D-DM-12` is deliberate, not an oversight: `create_team` atomically bootstraps a Team's first PersonRole row (its initial TeamLeadUser), and the never-leaderless guard (`D1.4-87`) permanently forbids removing a Team's last remaining one — so a Team can *never* actually reach zero PersonRole rows through the ordinary app flow, and blocking deletion on them the way Person's own check does would make this feature unusable for its own stated purpose (undoing a Team created moments ago, which by construction already has at least one member). Membership is treated as intrinsic to the Team, not an external reference to it: deleting a Team naturally ends everyone's membership in it, and unlike a Project/Component (real work) or unlike Person's own PersonRole rows (that *Person's* standing on other Teams, which must survive deleting some other Person), no actual work or other Person's data is lost by that. Full detail in `4_GuiClient/Plan.md` `D1.4-90`.
