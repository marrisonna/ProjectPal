...
- **A standing QA environment and full release-package pipeline.** Level 1 gets by on migration versioning, a verified backup, and a rollback script — real safety without a standing QA environment (`Claude/Level1_Implementation/8_ProductionDeployment/Plan.md`). The full Dev/QA/Prod promotion pipeline (§2 above) is deferred to this Level, once release frequency and more than one customer justify the standing infrastructure.
