...
- **Unlimited, non-paginated All Tasks grid.** Level 1's All Tasks screen shows Tasks 100 at a time with a pagination footer, the largest page size the free MUI DataGrid Community (MIT) tier allows (`D-Win-13` in `Claude/Requirements/UserInterfaceWindows.md`) — matching V1.2, which shows every Task at once with no paging, needs either a paid DataGridPro/Premium license (unlocks disabling pagination entirely, with the grid's existing row virtualization handling any row count) or a custom virtualized list built outside DataGrid's pagination feature; neither is committed to yet.
