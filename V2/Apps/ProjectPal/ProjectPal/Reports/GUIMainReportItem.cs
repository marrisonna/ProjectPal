...
namespace ProjectPal.Reports
{
    class GUIMainReportItem : CustomGUIControls.Grid.IGridItem
    {
        ...
        public void AddTaskData(double effort, double urgency, DBProjectPal.StatusValue? taskStatus)
        {
            m_taskCount++;
            ...
            if (taskStatus == DBProjectPal.StatusValue.Ready)
                m_ready++;

            if (taskStatus == DBProjectPal.StatusValue.InProgress)
                m_inProgess++;
        }
