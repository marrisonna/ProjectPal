    ...
    public partial class TaskDetail : Form, IView
    {
        ...
        void SetPermisions()
        {
            ...
            if (!fullEditAllowed)
            {
                ...
                if (Permissions.IsReadOnly || !underlyingObject.DBTask.Resources.Contains(DBTaskMan.Person.CurrentUser))
                {
                ...
                if (underlyingObject.Owner == DBTaskMan.Person.DBUser)
            ...
            else
            {
                ...
                {
                    string currentUser = DBTaskMan.Person.FindPersonFromDBLogin(underlyingObject.Owner).Name;
        ...
        public void DeleteRemark(CustomGUIControls.IDisplayItem remark)
        {
            ...
            TaskMan.Remarks.GUIRemark guiRemark = remark as TaskMan.Remarks.GUIRemark;
            if (guiRemark != null)
            {
                guiRemark.SaveToDB();
        ...
        public TaskDetail(DBTaskMan.Project ownerProject)
        {
            Init();

            DBTaskMan.Task newTask = DBTaskMan.Task.AddNewInstance("", m_affectedComponent);
            ...
            comboBoxStatus.Text = TaskMan.Tasks.GUITaskColumns.s_statusNotStarted;

            labelDateAdded.Text = newTask.DateAdded.ToString("dd-MMM-yyyy");
            labelStatusDate.Text = newTask.StatusDate.ToString("dd-MMM-yyyy");
            string ownerName = DBTaskMan.Person.FindPersonFromDBLogin(newTask.Owner).Name;
            ...
            comboBoxPriority.Text = TaskMan.Tasks.GUITaskColumns.s_priortyMed;
        ...
        public TaskDetail(DBTaskMan.Component ownerComponent)
        {
            Init();
        ...
        private void Redisplay()
        {
            ...
            if (underlyingObject != null && m_mode != WindowMode.New)
            {
                ...
                string ownerName = DBTaskMan.Person.FindPersonFromDBLogin(underlyingObject.Owner).Name;
...

                {
                    ...
                    m_affectedComponent = affectedCompObj as DBTaskMan.Component;
                ...
                radioButtonManDays.Checked = underlyingObject.DBTask.EffortType == DBTaskMan.EffortTypeValue.ManDays;
                radioButtonDuration.Checked = underlyingObject.DBTask.EffortType == DBTaskMan.EffortTypeValue.Duration;
        ...
        private void RedisplayAttachments()
        {
            ...
            if (underlyingObject != null)
            {
                ...
                foreach (DBTaskMan.Attachment currentAttachement in underlyingObject.Attachments)
                {
                    gridControlAttachments.AddDisplayItem(TaskMan.Attachments.GUIAttachment.GetInstanceFromDBAttachment(currentAttachement));
                }

                gridControlAttachments.ColumnWidth(TaskMan.Attachments.GUIAttachmentColumns.s_Name, -1, 250);
                gridControlAttachments.ColumnWidth(TaskMan.Attachments.GUIAttachmentColumns.s_DataType, -1, 60);
                gridControlAttachments.ColumnWidth(TaskMan.Attachments.GUIAttachmentColumns.s_CreateTime, -1);
                gridControlAttachments.ColumnWidth(TaskMan.Attachments.GUIAttachmentColumns.s_Size, 60);
                gridControlAttachments.ColumnWidth(TaskMan.Attachments.GUIAttachmentColumns.s_Owner, -1);

                gridControlAttachments.SetFilters();

                gridControlAttachments.SetDefaultSort(TaskMan.Attachments.GUIAttachmentColumns.s_CreateTime, ListSortDirection.Descending);
            }
        }
        ...
        private void RedisplayDependencies()
        {
            ...
            if (underlyingObject != null)
            {
                ...
                List<DBTaskMan.ITaskOrProject> preDependants = new List<DBTaskMan.ITaskOrProject>(underlyingObject.DBTask.ImmediatePreDependencies);
                preDependants.Sort(SortDependenciesByEndDate);

                foreach (DBTaskMan.ITaskOrProject preDependant in preDependants)
                {
                ...
                List<DBTaskMan.ITaskOrProject> postDependants = new List<DBTaskMan.ITaskOrProject>(underlyingObject.DBTask.ImmediatePostDependants);
                postDependants.Sort(SortDependenciesByDescription);

                foreach (DBTaskMan.ITaskOrProject postDependant in postDependants)
                {
        ...
        private int SortDependenciesByDescription(DBTaskMan.ITaskOrProject a, DBTaskMan.ITaskOrProject b)
        {
            return string.Compare(a.ObjectDescription, b.ObjectDescription);
        }

        private int SortDependenciesByEndDate(DBTaskMan.ITaskOrProject a, DBTaskMan.ITaskOrProject b)
        {
        ...
        private void RedisplayRemarks()
        {
            ...
            if (underlyingObject != null)
            {
                ...
                gridControlRemarks.SetColumns(TaskMan.Remarks.GUIRemarkColumns.Instance);

                tabPageRemark.Text = "Remarks (" + underlyingObject.Remarks.Count() + ")";

                foreach (DBTaskMan.Remark currentRemark in underlyingObject.Remarks)
                {
                    gridControlRemarks.AddDisplayItem(TaskMan.Remarks.GUIRemark.GetInstanceFromDBRemark(currentRemark));
                }

                gridControlRemarks.ColumnWidth(TaskMan.Remarks.GUIRemarkColumns.s_remark, -1);
                gridControlRemarks.ColumnWidth(TaskMan.Remarks.GUIRemarkColumns.s_owner, -1, 50);
                gridControlRemarks.ColumnWidth(TaskMan.Remarks.GUIRemarkColumns.s_createTime, -1);

                gridControlRemarks.SetFilters();

                gridControlRemarks.SetDefaultSort(TaskMan.Remarks.GUIRemarkColumns.s_createTime, ListSortDirection.Descending);
            }
        }

        public void ShowRemark(TaskMan.Remarks.GUIRemark guiRemark)
        {
        ...
        private void buttonCancel_Click(object sender, EventArgs e)
        {
            ...
            foreach (DBTaskMan.ITaskOrProject item in m_preDependenciesDeleted)
            {
                underlyingObject.DBTask.AddPreDependency(item);
            }
            m_preDependenciesDeleted.Clear();
            foreach (DBTaskMan.ITaskOrProject item in m_postDependenciesDeleted)
            {
        ...
        private void buttonOK_Click(object sender, EventArgs e)
        {
            ...
            if (radioButtonDuration.Checked)
                underlyingObject.DBTask.EffortType = DBTaskMan.EffortTypeValue.Duration;
            else
                underlyingObject.DBTask.EffortType = DBTaskMan.EffortTypeValue.ManDays;
            ...
            { // Owner
                ...
                DBTaskMan.Person userPerson = DBTaskMan.Person.FindPerson(comboBoxNewOwner.Text);
                if (userPerson != null)
                {
            ...
            //    TaskMan.Components.GUIComponent.Redisplay(underlyingObject.AffectedComponent);

            //    foreach (string projectName in underlyingObject.Projects)
            //    {
            //        DBTaskMan.Project newProject = DBTaskMan.Project.FindProject(projectName);

            //        TaskMan.Projects.GUIProject.Redisplay(newProject);
            //    }
            //    Close();
            //}
            //else
            ...
            //    DBTaskMan.Component origAffectedComponent = underlyingObject.AffectedComponent;
            //    underlyingObject.AffectedComponent = m_affectedComponent;
            ...
            //    {
            //        TaskMan.Components.GUIComponent.Redisplay(origAffectedComponent);
            //        TaskMan.Components.GUIComponent.Redisplay(underlyingObject.AffectedComponent);
            //    }
        ...
        private void buttonComponents_Click(object sender, EventArgs e)
        {
            ...
            foreach (DBTaskMan.Component aComponent in DBTaskMan.Component.TopLevelComponents)
            {
        ...
        private void AddComponentToTreeView(TreeNode root, DBTaskMan.Component theComponent, ref TreeNode selectedNode)
        {
            List<TreeNode> nodes = new List<TreeNode>();
            foreach (DBTaskMan.Component aComponent in theComponent.SubComponents)
            {
                TreeNode thisRoot = new TreeNode(aComponent.Name);
                thisRoot.Tag = aComponent;
        ...
        private void treeViewComponents_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            if (!m_ignoreSelect)
            {
                ...
                if (e.Node.Tag as DBTaskMan.Component != null)
                {
                    DBTaskMan.Component selectedComponent = e.Node.Tag as DBTaskMan.Component;
                ...
                if (e.Node.Tag as DBTaskMan.Project != null)
                {
                    DBTaskMan.Project selectedProject = e.Node.Tag as DBTaskMan.Project;
        ...
        DBTaskMan.Component m_affectedComponent;
        ...
        {
            List<TreeNode> nodes = new List<TreeNode>();
            foreach (DBTaskMan.Project aProject in DBTaskMan.Project.TopLevelProjects)
            {
        ...
        private bool HasNoPermissionOnThisAndAllSubProjects(DBTaskMan.Project theProject)
        {
            List<DBTaskMan.Project> projectsToCheck = new List<DBTaskMan.Project>(theProject.AllActiveSubProjects);
            projectsToCheck.Add(theProject);
            foreach (DBTaskMan.Project projectToCheck in projectsToCheck)
            {
        ...
        private void AddProjectToTreeView(TreeNode root, DBTaskMan.Project theProject)
        {
            List<TreeNode> nodes = new List<TreeNode>();
            foreach (DBTaskMan.Project aProject in theProject.SubProjects)
            {
                if (aProject.IsHidden)
                    continue;
        ...
        private void TaskDetail_DragEnter(object sender, DragEventArgs e)
        {
            if (m_viewImplementation.ItemsToDisplay.Count > 0)
            {
                ...
                if ((Permissions.IsAllowed(underlyingObject.DBTask.Owner, Permissions.EntityType.Task, Permissions.ChangeType.Edit) ||
                    (underlyingObject.DBTask.Resources.Contains(DBTaskMan.Person.CurrentUser))) &&
        ...
        private void TaskDetail_DragDrop(object sender, DragEventArgs e)
        {
            if (m_viewImplementation.ItemsToDisplay.Count > 0)
            {
                ...
                foreach (Utils.DragDrop.DroppedFile file in files.Files)
                {
                    ...
                    if (DBTaskMan.Attachment.AddNewInstance((m_viewImplementation.FirstItemToDisplay as GUITask).DBTask, file.Title, file.Type, file.From, file.TimeStamp, file.FileContents) != null)
                    {
        ...
        private void listBoxProjects_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                DBTaskMan.Project dbProject = DBTaskMan.Project.FindProject(selectedItem);
                if (dbProject != null)
                {
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            ...
            TaskMan.Remarks.RemarkWindow window = new Remarks.RemarkWindow();
            if (window.ShowDialog(this) == System.Windows.Forms.DialogResult.OK)
            {
                GUITask underlyingObject = m_viewImplementation.FirstItemToDisplay as GUITask;
                if (underlyingObject != null && underlyingObject.DBTask.TaskId >= 0)
                {
                    DBTaskMan.Remark newRemark = DBTaskMan.Remark.AddNewInstance(underlyingObject.DBTask, window.RemarkText);
                    TaskMan.Remarks.GUIRemark guiRemark = TaskMan.Remarks.GUIRemark.GetInstanceFromDBRemark(newRemark);
        ...
        private void listBoxDependency_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Delete || e.KeyCode == Keys.Back)
            {
                ...
                DBTaskMan.ITaskOrProject itemSelected = listboxUsed.SelectedItem as DBTaskMan.ITaskOrProject;
                if (itemSelected != null)
                {
                    ...
                    {
                        if ((itemSelected is DBTaskMan.Task &&
                               Permissions.IsAllowed((itemSelected as DBTaskMan.Task).Owner, Permissions.EntityType.Task, Permissions.ChangeType.Edit))
                             ||
                            (itemSelected is DBTaskMan.Project &&
                               Permissions.IsAllowed((itemSelected as DBTaskMan.Project).Owner, Permissions.EntityType.Project, Permissions.ChangeType.Edit))
                            )
                        {
        ...
        List<DBTaskMan.ITaskOrProject> m_preDependenciesDeleted = new List<DBTaskMan.ITaskOrProject>();
        List<DBTaskMan.ITaskOrProject> m_postDependenciesDeleted = new List<DBTaskMan.ITaskOrProject>();
        ...
        private void listBoxDependency_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (itemSelected != null)
            {
                if (itemSelected is DBTaskMan.Task)
                {
                    GUITask theGuiTask = GUITask.GetInstanceFromDBTask((DBTaskMan.Task)itemSelected);
                    TaskMan.Tasks.TaskDetail.ShowDetailWindow(theGuiTask);
                }

                if (itemSelected is DBTaskMan.Project)
                {
                    Projects.GUIProject guiProject = Projects.GUIProject.GetInstanceFromDBProject((DBTaskMan.Project)itemSelected);
                    Projects.ProjectDetail.GetAndShowDetailWindow(guiProject);
                }
        ...
        private void listBoxDependsUpon_DragDrop(object sender, DragEventArgs e)
        {
            ...
            if (draggedTask != null)
            {
                ...
                DBTaskMan.Task postTask = underlyingObject.DBTask;
                DBTaskMan.Task preTask = draggedTask.DBTask;
                if (preTask == postTask)
                    return;
            ...
            TaskMan.Projects.GUIProject draggedProject = draggedObject as TaskMan.Projects.GUIProject;
            if (draggedProject != null)
            {
                GUITask underlyingObject = m_viewImplementation.FirstItemToDisplay as GUITask;

                DBTaskMan.Task postTask = underlyingObject.DBTask;
                DBTaskMan.Project preProject = draggedProject.DBProject;
        ...
        private void listBoxDependants_DragDrop(object sender, DragEventArgs e)
        {
            ...
            if (draggedTask != null)
            {
                ...
                DBTaskMan.Task preTask = underlyingObject.DBTask;
                DBTaskMan.Task postTask = draggedTask.DBTask;
                if (preTask == postTask)
                    return;
            ...
            if (draggedProject != null)
            {
                ...
                DBTaskMan.Task preTask = underlyingObject.DBTask;
                DBTaskMan.Project postProject = draggedProject.DBProject;
