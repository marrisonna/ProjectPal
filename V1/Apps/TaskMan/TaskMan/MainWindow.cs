...
namespace TaskMan
{
    public partial class MainWindow : Form
    {
        ...
        private void SetUpReport()
        {
            ...
            columnOrder.Add(TaskMan.Reports.GUIMainReportItemColumns.s_Person);
            columnOrder.Add(TaskMan.Reports.GUIMainReportItemColumns.s_NumberOfOpenTasks);
            columnOrder.Add(TaskMan.Reports.GUIMainReportItemColumns.s_NumberOfReadyTasks);
            columnOrder.Add(TaskMan.Reports.GUIMainReportItemColumns.s_NumberOfInProgressTasks);
            columnOrder.Add(TaskMan.Reports.GUIMainReportItemColumns.s_Effort);
            columnOrder.Add(TaskMan.Reports.GUIMainReportItemColumns.s_AveUrgency);
            columnOrder.Add(TaskMan.Reports.GUIMainReportItemColumns.s_MaxUrgency);

            gridControl.SetColumnOrder(columnOrder);

            gridControl.SetColumns(TaskMan.Reports.GUIMainReportItemColumns.Instance);

            gridControl.ColumnWidth(TaskMan.Reports.GUIMainReportItemColumns.s_Person, 80);
            gridControl.ColumnWidth(TaskMan.Reports.GUIMainReportItemColumns.s_NumberOfOpenTasks, 70);
            gridControl.ColumnWidth(TaskMan.Reports.GUIMainReportItemColumns.s_NumberOfReadyTasks, 70);
            gridControl.ColumnWidth(TaskMan.Reports.GUIMainReportItemColumns.s_NumberOfInProgressTasks, 70);
            gridControl.ColumnWidth(TaskMan.Reports.GUIMainReportItemColumns.s_Effort, 90);
            gridControl.ColumnWidth(TaskMan.Reports.GUIMainReportItemColumns.s_AveUrgency, 90);
            //gridControl.ColumnWidth(TaskMan.Reports.GUIMainReportItemColumns.s_MaxUrgency, 80);
        ...
        private void ShowReport()
        {
            ...
            SortedDictionary<string, TaskMan.Reports.GUIMainReportItem> reportItems = new SortedDictionary<string, TaskMan.Reports.GUIMainReportItem>();

            TaskMan.Reports.GUIMainReportItem total = new Reports.GUIMainReportItem(TotalRowLabel);

            foreach (DBTaskMan.Task task in DBTaskMan.Task.AllInstances)
            {
                ...
                TaskMan.Tasks.GUITask guiTask = TaskMan.Tasks.GUITask.GetInstanceFromDBTask(task);
                double urgency = guiTask.Urgency;
                foreach (IResource person in task.TeamResources)
                {
                    TaskMan.Reports.GUIMainReportItem reportItem;
        ...
        void RefreshTimer_Tick(object sender, EventArgs e)
        {
            ...
            ApplicationTaskMan.Instance.RefreshFromDatabase();
        ...
        private void buttonShowTasks_Click(object sender, EventArgs e)
        {
            if (!Permissions.IsSuperUser)
                ShowTaskListWindow(DBTaskMan.Person.CurrentUser.Name);
            else
        ...
        {
            bool urgencyCacheOrig = TaskMan.Tasks.GUITask.UrgencyCachingEnabled;
            ...
            foreach (Task task in tasks)
            {
                ...
                //    continue;
                tasksForGUI.Add(TaskMan.Tasks.GUITask.GetInstanceFromDBTask(task));
            }

            Tasks.TaskWindow theTaskWindow = new Tasks.TaskWindow(tasksForGUI, resourceFilter);

            theTaskWindow.SetGridDoubleClickFunction(TaskMan.Tasks.TaskDetail.ShowDetailWindow);

            theTaskWindow.Show();

            TaskMan.Tasks.GUITask.UrgencyCachingEnabled = urgencyCacheOrig;
        }

        static public void DoubleClickRow(CustomGUIControls.IDisplayItem objectToDisplay)
        {
            TaskMan.Reports.GUIMainReportItem selectedItem = objectToDisplay as TaskMan.Reports.GUIMainReportItem;
            if (selectedItem != null)
            {

                if (selectedItem.Person != TotalRowLabel && selectedItem.Person != DBTaskMan.Task.OtherResource)
                {
                    ShowTaskListWindow(selectedItem.Person);
                }
                else if (selectedItem.Person == DBTaskMan.Task.OtherResource)
                {
                    List<string> otherPeople = new List<string>();
                    foreach (DBTaskMan.Person aPerson in DBTaskMan.Person.AllActiveInstances)
                    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            foreach (Project project in projects)
            {
                if (project.IsHidden)
                    continue;
                projectsForGUI.Add(TaskMan.Projects.GUIProject.GetInstanceFromDBProject(project));
            }

            Projects.ProjectWindow theProjectWindow = new Projects.ProjectWindow(projectsForGUI);

            theProjectWindow.SetGridDoubleClickFunction(TaskMan.Projects.ProjectDetail.ShowDetailWindow);
        ...
        private void buttonRefresh_Click(object sender, EventArgs e)
        {
            ...
            ApplicationTaskMan theApp = ApplicationTaskMan.Instance;
        ...
        private void manageUsersToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                m_windowManagePeopleInstance = new TaskMan.People.FormManagePeople(this);
        ...
        internal TaskMan.People.FormManagePeople m_windowManagePeopleInstance = null;
    }
}
