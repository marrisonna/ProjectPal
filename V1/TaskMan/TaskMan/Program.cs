...
namespace TaskMan
{
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                //Utils.DatabaseCE theDatabaseCe = new Utils.DatabaseCE("TaskMan");

                //Utils.DatabaseCE.Test();


                //Utils.Database theDatabase = new Utils.Database("TaskMan");
                ...
                if (args.Length > 0)
                {
                    if (args.Contains("-refreshAttachments"))
                    {
                        if (Permissions.ThisUserType == Permissions.UserLevel.SuperUser)
                        {
                            ...
                            DBTaskMan.Attachment.ReSaveAll();
                ...
                if (!currentUserType.HasValue)
                {
                    ...
                    DBTaskMan.Person currentUser = DBTaskMan.Person.FindPersonFromDBLogin(DBAccess.DBObjectBase.DBUser);
                    if (currentUser != null)
                ...
                //IEnumerable<DBTaskMan.Task> tasks = DBTaskMan.Task.AllInstances;
                //IEnumerable<DBTaskMan.Attachment> attachments = DBTaskMan.Attachment.AllInstances;
        ...
        {
            DBTaskMan.Project.HideCompletedProjects = ConfigSettings.Instance.HideCompletedProjects;
        }
