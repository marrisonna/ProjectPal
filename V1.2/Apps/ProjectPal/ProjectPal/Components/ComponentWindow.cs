...
namespace ProjectPal.Components
{
    public partial class ComponentWindow : Form
    {
        ...
        private static Dictionary<DBProjectPal.Component, ComponentWindow> s_allInstances = new Dictionary<DBProjectPal.Component, ComponentWindow>();
        ...
        private ComponentWindow(GUIComponent rootComponent)
        {
            ...
            m_theComponentsStackControl = new ComponentStackControl(ProjectPal.Tasks.TaskDetail.ShowDetailWindow);
        ...
        private void Redisplay()
        {
            ...
            List<DBProjectPal.Component> components = null;
            bool openSubComponents = false;
            if (m_rootComponent == null)
            {
                components = new List<DBProjectPal.Component>(DBProjectPal.Component.TopLevelComponents);
            ...
            else
            {
                ...
                DBProjectPal.Person owner = DBProjectPal.Person.FindPersonFromDBLogin(m_rootComponent.Owner);
                string personName = owner == null ? "" : owner.Name;
                labelOwner.Text = "Owner: " + personName;

                components = new List<DBProjectPal.Component>();
                ...
                foreach (DBProjectPal.Attachment currentAttachement in m_rootComponent.Attachments)
                {
            ...
            {
                DBProjectPal.Component currentComponentToBeDisplayed = index < components.Count ? components[index] : null;
        ...
        DBProjectPal.Component m_rootComponent = null;

        private static int SortComponents(DBProjectPal.Component a, DBProjectPal.Component b)
        {
        ...
        private void gridControlAttachments_DragDrop(object sender, DragEventArgs e)
        {
            if (Permissions.IsAllowed(RootOwner, Permissions.EntityType.Component, Permissions.ChangeType.Edit))
            {
                ...
                {
                    DBProjectPal.Attachment.AddNewInstance(m_rootComponent, file.Title, file.Type, file.From, file.TimeStamp, file.FileContents);
                    Functions.ClearDisplayCaches();
        ...
        {
            DBProjectPal.Component parentComponent = m_rootComponent.Parent;
        ...
        private void RedisplayGantt()
        {
            ...
            HashSet<DBProjectPal.Task> topLevelTasks = new HashSet<DBProjectPal.Task>();
            ...
            if (m_rootComponent == null)
            {
                List<DBProjectPal.Component> components = new List<DBProjectPal.Component>(DBProjectPal.Component.TopLevelComponents);
                components.Sort(SortComponents);
            ...
            {
                DBProjectPal.Component component = m_rootComponent;
                foreach (DBProjectPal.Task componentTask in m_rootComponent.Tasks)
        ...
        private void buttonNewComponent_Click(object sender, EventArgs e)
        {
            ...
            if (result == System.Windows.Forms.DialogResult.OK)
            {
                ...
                DBProjectPal.Component theNewComponent = DBProjectPal.Component.AddNewInstance(newComponentName, m_rootComponent);
                Functions.ClearDisplayCaches();
                if (m_rootComponent != null)
                    ProjectPal.Components.GUIComponent.Redisplay(m_rootComponent);
                else
                {
                    Redisplay();
                }
