...
namespace DBTaskMan
{
    public class Task : DBAccess.DBObject<Task>, ISearchable, ITaskOrProject
    {
        ...
        //    DBTaskMan.Internal.LinkTable_Task2Project.RemoveLinksToPrivateProjects();
        ...
        public List<IResource> TeamResources
        {
            get
            {
                ...
                foreach (DBTaskMan.IResource resource in Resources)
                {
                    DBTaskMan.Person resourceName = DBTaskMan.Person.OtherPerson;
                    DBTaskMan.Person realPerson = DBTaskMan.Person.FindPerson(resource.Name);
                ...
                {
                    resourcesForThisTask.Add(DBTaskMan.Person.UnassignedResource);
                }
        ...
        public double? Duration
        {
            get
            {
                ...
                if (m_effortType.Value == EffortTypeValue.ManDays)
                {
                    ...
                    foreach (DBTaskMan.Person assignedResource in this.Resources)
                    {
        ...
        private void PerserveDepantsStartDates()
        {
            if (this.EndDate.HasValue)
            {
                foreach (ITaskOrProject postItem in this.PostDependencies)
                {
                    ...
                    DBTaskMan.Project postProject = postItem as DBTaskMan.Project;
                    if (postTask != null)
                    {
            ...
            DBTaskMan.Project parentProject = this.Project;
