...
namespace TaskMan
{
    public partial class AdminWindow : Form
    {
        public AdminWindow()
        {
            ...
            if (DBTaskMan.Person.CurrentUser != null)
                currentUser = DBTaskMan.Person.CurrentUser.Name;
        ...
        {
            textBoxLevel.Text = DBTaskMan.Person.FindPerson(comboBoxUsers.Text).UserType.ToString();
        }
        ...
        private void buttonRestartAsDifferentUser_Click(object sender, EventArgs e)
        {
            ...
            Functions.AppendAdminPassword(ref arguments);
            arguments += "," + DBTaskMan.Person.FindPerson(comboBoxUsers.Text).DBLogin.Trim();
        ...
        {
            DBTaskMan.Functions.LoadAllTypes();
            try
            {
                if (Utils.DatabaseBase.DBType == Utils.DatabaseBase.DBTypeValues.SQLServer)
                {
        ...
        {
            foreach (DBTaskMan.Attachment attachement in DBTaskMan.Attachment.AllInstances)
            {
        ...
        private void buttonExportAttachements_Click(object sender, EventArgs e)
        {
            ...
            foreach (DBTaskMan.Attachment attachement in DBTaskMan.Attachment.AllInstances)
            {
                ...
                if (attachement.Task != null)
                    directory = "Task_" + (int?)DBTaskMan.Attachment.GetDatabaseIdForWritingToDatabase(attachement.Task);
                if (attachement.Component != null)
                    directory = "Component_" + (int?)DBTaskMan.Attachment.GetDatabaseIdForWritingToDatabase(attachement.Component);
                if (attachement.Project != null)
                    directory = "Project_" + (int?)DBTaskMan.Attachment.GetDatabaseIdForWritingToDatabase(attachement.Project);
